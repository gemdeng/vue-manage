...
export default {
  ...
  methods: {
    ...
    addAttrValue() {
      //像属性值的数组里面添加元素
      //attrId:是你相应的属性的id,目前而言我们是添加属性的操作，还没有相应的属性的id,目前而言带给服务器的id位undefined
      //valueName:相应的属性值的名称
      this.attrInfo.attrValueList.push({
        attrId: this.attrInfo.id, //对于修改某一个属性的时候，可以在已有的属性值基础之上新增新的属性值（新增属性值的时候，需要把已有的属性的id带上）
        valueName: "",
        flag: true, //flag属性：给每一个属性值添加一个标记flag,用于切换查看模式与编辑模式，好处是每一个属性值可以控制自己的模式切换，当前的flag属性，响应式数据（数据变化视图跟着变化）
      });
      this.$nextTick(() => {
    ...
    updateAttr(row) {
      ...
      this.attrInfo.attrValueList.forEach((item) => {
        ...
        //第一个参数：对象 第二个参数：添加新的响应式属性 第三个参数：新的属性值
    ...
    toLook(row) {
      ...
      //当前的编辑模式变为查看模式【让input,消失，显示span】
      row.flag = false;
    },
    //点击span的回调，变为编辑模式
    toEdit(row, index) {
      row.flag = true;
      //获取input节点，实现自动聚焦
      //需要注意：点击span的时候，切换为input变为编辑模式，但是需要注意，对于浏览器，页面重绘与重拍耗时间的
    ...
    async addOrUpdateAttr(date) {
      //整理参数：1.如果用户添加很多属性值，且属性值为空的不应该提交给服务器
      //2.提交给服务器当中不应该出现flag字段
