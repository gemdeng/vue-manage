  ...
  <div>
    <el-form ref="form" label-width="80px" :model="spu">
      ...
      <el-form-item label="SPU图片">
        <!-- 上传图片：action图片上传的地址 list-type文件列表的类型 on-preview图片预览的时候会触发 on-remove当删除图片的时候会触发  file-list照片墙需要展示的数据【数组；数组里面的元素务必要有name、url属性】   -->
        <el-upload
...
export default {
  data() {
    return {
      ...
      dialogVisible: false,
      //存储SPU信息属性
      //spu属性，初始化是一个空的对象，在修改SPU的时候，会向服务器发请求，返回SPU信息（对象），在修改的时候可以利用服务器返回的这个对象收集最新的数据提交给服务器进行修改
      //添加SPU，如果是添加SPU的时候并没有向服务器发请求，数据收集到spu，收集的字段要参考文档
      spu: {
        //三级分类的id
        category3Id: 0,
        //描述
  ...
  methods: {
    ...
    handleRemove(file, fileList) {
      ...
      //收集照片墙图片的数据
      //对于已有的图片【照片墙中显示的图片，有name、url字段的】，因为照片墙显示数据务必要有这两个属性，对于服务器而言，它不需要name与url字段，将来对于已有的图片的数据在提交给服务器的时候，需要处理
